// Ipoh page specific JavaScript

// Global map initialization function for Google Maps callback
function initIpohMap() {
    const mapOptions = {
        center: new google.maps.LatLng(4.5975, 101.0901), // Ipoh coordinates
        zoom: 13,
    ...
    const map = new google.maps.Map(document.getElementById('ipoh-map'), mapOptions);

    // Add markers for food hotspots
    const locations = [
        {
            title: 'Lou Wong Restaurant',
            position: { lat: 4.5958, lng: 101.0779 },
            info: 'Daily, 10:30am - 9:30pm<br>Famous for: Bean sprouts chicken, soy sauce chicken'
        },
        {
            title: 'Thean Chun Coffee Shop',
            position: { lat: 4.5978, lng: 101.0763 },
            info: 'Daily, 8am - 4:30pm<br>Famous for: Ipoh hor fun, caramel egg custard'
        },
        {
            title: 'Sin Eng Heong',
            position: { lat: 4.5943, lng: 101.0821 },
            info: 'Daily, 8am - 5pm<br>Famous for: Heong peng (fragrant biscuits), egg biscuits'
        }
    ];
...
function loadComments() {
    ...
    const ipohComments = JSON.parse(localStorage.getItem('ipohComments')) || [];

    if (ipohComments.length === 0) {
        commentsContainer.innerHTML = '<p>No comments yet. Be the first to share your experience!</p>';
        return;
    }

    let commentsHTML = '';
    ipohComments.forEach(comment => {
        commentsHTML += `
            <div class="comment">
...
function setupCommentForm() {
    ...
    if (commentForm) {
        commentForm.addEventListener('submit', function (e) {
            ...
            // Add new comment
            ipohComments.unshift(newComment);

            // Save back to local storage
            localStorage.setItem('ipohComments', JSON.stringify(ipohComments));
